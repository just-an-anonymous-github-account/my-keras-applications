...
def block1(x, filters, kernel_size=3, stride=1,
           conv_shortcut=True, name=None, dilation_rate=1):
    """A residual block.
    ...
    x = layers.Conv2D(filters, 1, strides=stride, name=name + '_1_conv', dilation_rate=(dilation_rate, dilation_rate))(x)
...
def stack1(x, filters, blocks, stride1=2, name=None):
    ...
    """
    if name == 'conv4d':
        x = block1(x, filters, stride=1, name=name + '_block1', dilation_rate=2)
    elif name == 'conv5d':
        x = block1(x, filters, stride=1, name=name + '_block1', dilation_rate=2)
    else:
...
def ResNet50(include_top=True,
    ...
    def stack_fn(x):
        ...
        if magic_alan_time:
            x = stack1(x, 256, 23, name='conv4d')
            x = stack1(x, 512, 3, name='conv5d')
        else:
